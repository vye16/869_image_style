import tensorflow as tf
import numpy as np

from scipy.io import loadmat

netfile = 'imagenet-vgg-verydeep-19.mat'

'''conv net layers:
    conv1_1, relu1_1, conv1_2, relu1_2, pool1,

    conv2_1, relu2_1, conv2_2, relu2_2, pool2,

    conv3_1, relu3_1, conv3_2, relu3_2,
    conv3_3, relu3_3, conv3_4, relu3_4, pool3,

    conv4_1, relu4_1, conv4_2, relu4_2,
    conv4_3, relu4_3, conv4_4, relu4_4, pool4,

    conv5_1, relu5_1, conv5_2, relu5_2,
    conv5_3, relu5_3, conv5_4, relu5_4
'''


def get_network(image):
    data = loadmat(netfile);
    vgg_layers = data['layers'].flatten()
    avg_image = data['normalization']['averageImage'][0,0]
    channel_avg = avg_image.mean(axis=(0,1))

    network = {}

    input_layer = image
    for layer in vgg_layers:
        name = str(layer['name'][0,0][0])
        if name.startswith('conv'):
            filters, bias = layer['weights'][0,0].flatten()
            # tf axes for filters [height, width, in, out]
            filters = filters.transpose((1,0,2,3))
            conv = tf.nn.conv2d(input_layer,
                    tf.constant(filters),
                    strides=[1,1,1,1],
                    padding='SAME')
            network[name] = tf.nn.bias_add(conv, bias.reshape(-1))

        elif name.startswith('pool'):
            # the paper uses average pooling instead of max pooling
            # downsample by 2 in height and width
            network[name] = tf.nn.max_pool(input_layer,
                    ksize=[1,2,2,1], strides=[1,2,2,1],
                    padding='SAME')

        elif name.startswith('relu'):
            network[name] = tf.nn.relu(input_layer)

        else: # we're not using fully connected layers
            continue

        # update input layer
        input_layer = network[name]
    return network, channel_avg
